...
<template>
  <div class="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4 xl:gap-5">
    <div
      v-for="(card, i) in cards"
      :key="i"
      class="group relative overflow-hidden rounded-2xl border border-white/10 bg-surface/80 p-3 shadow-[0_12px_30px_rgba(15,17,23,0.28)] transition-all duration-300 ease-out hover:-translate-y-1 hover:border-white/20 hover:shadow-[0_18px_40px_rgba(15,17,23,0.35)] sm:p-4"
    >
      ...
      <div class="relative z-10 flex items-center justify-between gap-2.5">
        <div class="min-w-0 flex-1">
          <p class="text-[9px] font-semibold uppercase tracking-[0.14em] text-white/40 sm:text-[10px] xl:text-[11px]">{{ card.title }}</p>
          <p class="mt-2 text-[0.92rem] font-bold leading-tight tracking-[-0.04em] text-white tabular-nums sm:text-[1rem] xl:text-[1.2rem]">
            {{ card.value }}
          </p>
        </div>

        <div
          :class="`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border sm:h-9 sm:w-9 xl:h-10 xl:w-10 ${colorStyleOptions[card.theme].iconBox}`"
        >
          <component :is="card.icon" class="h-3.5 w-3.5 sm:h-4 sm:w-4 xl:h-4 xl:w-4" />
        </div>
      </div>
    </div>
  </div>
